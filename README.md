Projektbeschreibung:
Unser Projekt "Furari" ist ein Spiel, in welchem man einen Hacker spielt und Informationen über eine Person herausfindet.
Es soll auf seinen eigenen digital footprint aufmerksam machen und zeigen, wie viel man über online geteilte Dinge über einen erfahren kann. Informationen,
die man so nicht jeden fremden zeigen wollen würde, sollte man auch nicht online sorglos auf Instagram posten beispielsweise.

Nutzungsanleitung:
1. Das Spiel beginnt mit der Aufgabe, den Namen der Person in das Eingabefeld einzugeben. (Enter-Taste zum Bestätigen) <details><summary>Lösung:</summary>**(Ava Smith)**</details>
2. Im ersten Raum "Foyer" muss man sich die Bilder auf der Wand anschauen, um das Geburtsdatum von der Person rauszufinden. Man kommt weiter, indem man die richtige Lösung ins eingabefeld eintippt und die Enter-Taste drückt.. <details><summary>Lösung:</summary>**(27.06.2003)**</details>
3. Im zweiten Raum "Livingroom" muss man das Freundschaftsbuch aufheben, durchblättern und die Fotos richtig im Stammbaum einordnen. Wenn die Lösung stimmt, kommt man bei Mausklick auf die Tür in den nächtsen Raum .
4. Im dritten Raum "Office" klickt man auf das Bild über den Schreibtisch, liest die Email-Adresse vom Lebenslauf ab, und tippt diese in das Eingabe-Feld ein.
5. Im vierten Raum "Bedroom" ...
6. Im fünften Raum "Kitchen" ...
7. (Chat)
Eingabe erfolt durch Maus und Tastatur.

Screenshot:
<img width="1877" height="1253" alt="Furari_Start" src="https://github.com/user-attachments/assets/b55ccc81-ee41-4f9c-8b54-c6238c429808" />

Genaue Aufgabenverteilung:
Michael:

Sina:


Valentina:
Raumsteuerung (Tür-Buttons zum Weiterkommen, bools wenn Puzzles erledigt sind
Startanimation und Puzzles vom Start und Raum 1-3 (Namen & Geburtsdatum & E-mail von Ava eingeben, Instagram von Ava einsehen (das Scirpts für die Einagbefelder & Bildanzeige sind von Sina))
Wohnzimmerpuzzle (Bilder dem Stammbaum zuweisen)
Alle Grafiken (Screenshots aus Sims 4 + Filter aus Photoshop)


Development:
Version: Unity 6000.0.57f1
Programmiersprache: C#
Betriebssystem: Windows

Destination Platfrom:
WebGL
